...
class CountDownTimerCPT extends Cpt {
	...
	public function countdown_shortcode( $attrs ) {
		...
		if ( ! empty( $attrs['datetime'] ) ) {
			$timestamp = (int) sanitize_text_field( wp_unslash( $attrs['datetime'] ) );
		...
		if ( ! isset( $arrival_time ) || ! is_numeric( $arrival_time ) || (int) $arrival_time !== $arrival_time ) {
			try {
		...
		?>
		<div id="<?php echo esc_attr( self::$plugin_info['classes_prefix'] . '-countdown-timer-' . $post_id ); ?>" class="flipdown-size-sm <?php echo esc_attr( self::$plugin_info['classes_prefix'] . '-countdown-timer-container' ); ?>">
		...
		<?php
		$GLOBALS[ self::$plugin_info['classes_prefix'] . '-countdown-timer-shortcode' ] = true;
		return ob_get_clean();
	}
	...
	private function is_valid_timestamp( $timestamp ) {
		try {
			return (bool) \DateTime::createFromFormat( 'U', $timestamp );
		} catch ( \Exception $e ) {
			return false;
		}
	}
	...
	private function get_timer_title( $post_id ) {
		...
		ob_start();
		?>
		<<?php echo esc_attr( $timer_title_tag ); ?> class="<?php echo esc_attr( self::$plugin_info['classes_prefix'] . '-timer-title' ); ?>"><?php printf( esc_html__( '%s', 'simple-countdown' ), $timer_title ); ?></<?php echo esc_attr( $timer_title_tag ); ?>>
		<?php
		return ob_get_clean();
	}
